...
namespace RastreoPaquetes.Clases
{
    public class Dhl : IPaqueteria
    {
        ...
        public Dhl(IMedioTransporte[] _lstMedioTransportes, IMargenUtilidad _margenUtilidad) {
            LstMediosTransporte = _lstMedioTransportes;
            MargenUtilidad = _margenUtilidad;
            TiempoReparto = new Dictionary<string, decimal>
            {
                { MedioTransporteEnum.Maritimo.ToString().ToUpper(), 20m },
                { MedioTransporteEnum.Terrestre.ToString().ToUpper(), 12m },
                { MedioTransporteEnum.Aereo.ToString().ToUpper(), 3m }
            };
        }
        ...
        private IMedioTransporte ObtenerMedioTransporte(string _nombreMedioTransporte)
        {
            ...
            {
                throw new MedioTransporteException($"Dhl no ofrece el servicio de transporte {_nombreMedioTransporte}, te recomendamos cotizar en otra empresa.");
            }
